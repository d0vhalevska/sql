...
SELECT name AS 'Bands Name' FROM bands;
...
SELECT b.name AS 'Band Names', COUNT(a.id) AS 'num_albums' 
FROM bands AS b
JOIN albums as a ON b.id = a.band_id
GROUP BY a.band_id
HAVING num_albums is NOT NULL;
...
SELECT bands.name AS 'Band', albums.band_id, COUNT(songs.album_id) as 'Number of Songs'
FROM bands
JOIN albums ON bands.id = albums.band_id
JOIN songs ON albums.id = songs.album_id
GROUP BY bands.id;